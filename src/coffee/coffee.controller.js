"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CoffeeController = void 0;
var common_1 = require("@nestjs/common");
var CoffeeController = /** @class */ (function () {
    function CoffeeController() {
    }
    CoffeeController.prototype.findAll = function () {
        return 'i find all coffee';
    };
    CoffeeController.prototype.findOne = function (id) {
        return "i find ".concat(id, " count of coffee ");
    };
    CoffeeController.prototype.create = function (body) {
        return body;
    };
    CoffeeController.prototype.update = function (id, body) {
        return "i changed ".concat(id, " counts coffee");
    };
    CoffeeController.prototype.remove = function (id) {
        return "this action remove ".concat(id, " counts coffee ");
    };
    __decorate([
        (0, common_1.Get)()
    ], CoffeeController.prototype, "findAll", null);
    __decorate([
        (0, common_1.Get)(':id'),
        __param(0, (0, common_1.Param)('id'))
    ], CoffeeController.prototype, "findOne", null);
    __decorate([
        (0, common_1.Post)(),
        (0, common_1.HttpCode)(common_1.HttpStatus.FORBIDDEN),
        __param(0, (0, common_1.Body)('name'))
    ], CoffeeController.prototype, "create", null);
    __decorate([
        (0, common_1.Patch)(':id'),
        __param(0, (0, common_1.Param)('id')),
        __param(1, (0, common_1.Body)())
    ], CoffeeController.prototype, "update", null);
    __decorate([
        (0, common_1.Delete)(':id'),
        __param(0, (0, common_1.Param)('id'))
    ], CoffeeController.prototype, "remove", null);
    CoffeeController = __decorate([
        (0, common_1.Controller)('coffee')
    ], CoffeeController);
    return CoffeeController;
}());
exports.CoffeeController = CoffeeController;
//# sourceMappingURL=coffee.controller.js.map